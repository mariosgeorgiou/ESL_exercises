...
\begin{enumerate}
	...
	\item Consider a linear regression model with $p$ parameters, fit by least
	      ...
	      \begin{soln}
		      ...
		      and $T$ be the testing set. Moreover, let $f(S,\beta)$ be the
		      function we want to minimize (in our case it will be the RSS or
		      its normalized version, the mean squared error $\MSE$). Let
		      $\beta_S = \argmin_\beta f(S,\beta)$ and observe that for any
		      $\beta$, it holds that $f(S,\beta_S)\le f(S,\beta)$. We have
		      \begin{equation}
			      \begin{split}
				      E_{S,T}[f(T,\beta_S)] &= E_S E_{T/S}[f(T,\beta_S)] \\
				      & \ge E_S E_{T/S}[f(T,\beta_T)] \\
				      &= E_S E_T[f(T,\beta_T)] \\
				      &= E_S[f(S,\beta_S)],
			      \end{split}
		      \end{equation}
		      where the second line comes from the above inequality and the
		      third line comes from the independence between $S$ and $T$.
